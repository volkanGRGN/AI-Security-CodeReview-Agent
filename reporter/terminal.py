...
""")
    ...
    def print_findings_detail(self, findings: list):
        ...
        for category, cat_findings in sorted(by_category.items()):
            ...
            sorted_findings = sorted(cat_findings, key=lambda x: {'CRITICAL': 0, 'HIGH': 1, 'MEDIUM': 2, 'LOW': 3, 'INFO': 4}.get(x['severity'], 5))
            for f in sorted_findings:
                color = SEVERITY_COLORS.get(f['severity'], '')
                icon = SEVERITY_ICONS.get(f['severity'], '')
                print(f"    {color}{icon} {f['severity']:<10}{Style.RESET_ALL} {f['name']}")
                print(f"             {Fore.WHITE}{f['file']}:{f['line']}{Style.RESET_ALL}")
